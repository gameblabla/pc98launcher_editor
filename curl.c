...
/* Sadly this madness was needed due to Mobygames's strict filters */
const ExceptionMapping exceptions[] = {
    ...
    {"Viper GTS RS", "Viper+GTS"},
    // People may not really know the full name and just as Touhou 1,2... etc so this is needed and Mobygames didn't add any aliases to them
    {"Touhou 5", "Mystic+Square"},
    {"Touhou5", "Mystic+Square"},
    {"Touhou5", "Mystic+Square"},
    {"Touhou4", "Lotus+Land+Story"},
    {"Touhou 4", "Lotus+Land+Story"},
    {"Touhou4", "Lotus+Land+Story"},
    {"Touhou 4", "Lotus+Land+Story"},
    {"Touhou1", "Highly+Responsive+to+Prayers"},
    {"Touhou 1", "Highly+Responsive+to+Prayers"},
    {"Touhou 3", "The+Phantasmagoria+of+Dim.Dream"},
    {"Touhou3", "The+Phantasmagoria+of+Dim.Dream"},
    {"Touhou 2", "Story+of+Eastern+Wonderland"},
    {"Touhou2", "Story+of+Eastern+Wonderland"},
    // Panic Bomber may land on the virtual boy port instead
    {"Panic Bomber", "Bomberman+Panic+Bomber"},
    {"PanicBomber", "Bomberman+Panic+Bomber"},
    // Add more exceptions as needed
};
const size_t exception_count = sizeof(exceptions) / sizeof(exceptions[0]);

char *find_exception(const char *str) {
    for (size_t i = 0; i < exception_count; i++) {
        if (strcasecmp(str, exceptions[i].original) == 0) {
            return strdup(exceptions[i].mapped);
        }
    }
    return NULL;
}
...
char *url_encode(const char *str) {
    // First, check if the input string matches any exception
    char *exception_match = find_exception(str);
    if (exception_match) {
        return exception_match;
    }

    // If not, proceed with the custom encoding
